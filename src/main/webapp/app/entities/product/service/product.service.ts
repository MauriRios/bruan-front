...
type RestOf<T extends IProduct | NewProduct> = Omit<T, 'createDate' | 'lastModifyDate'> & {
...
export class ProductService {
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...product,
      createDate: product.createDate?.toJSON() ?? null,
      lastModifyDate: product.lastModifyDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...restProduct,
      createDate: restProduct.createDate ? dayjs(restProduct.createDate) : undefined,
      lastModifyDate: restProduct.lastModifyDate ? dayjs(restProduct.lastModifyDate) : undefined,
    };
  }
