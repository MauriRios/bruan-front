...
type RestOf<T extends ICategory | NewCategory> = Omit<T, 'createDate' | 'lastModifyDate'> & {
...
export class CategoryService {
  ...
  protected convertDateFromClient<T extends ICategory | NewCategory | PartialUpdateCategory>(category: T): RestOf<T> {
    return {
      ...category,
      createDate: category.createDate?.toJSON() ?? null,
      lastModifyDate: category.lastModifyDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCategory: RestCategory): ICategory {
    return {
      ...restCategory,
      createDate: restCategory.createDate ? dayjs(restCategory.createDate) : undefined,
      lastModifyDate: restCategory.lastModifyDate ? dayjs(restCategory.lastModifyDate) : undefined,
    };
  }
